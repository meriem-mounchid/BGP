#!/bin/sh
container_name=$(/usr/bin/docker ps | grep routeur_cel-oiri_3 | awk 'NF{print $NF}')
if [ -z $container_name ];
then
	echo ""
else
	...
	/usr/bin/docker exec  ${container_name} sh -c "ip link add name vxlan10 type vxlan id 10 dev eth0 group 239.1.1.1 dstport 4789"
