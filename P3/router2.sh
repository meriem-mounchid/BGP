...
container_name=$(/usr/bin/docker ps | grep router_ramoukha_2| awk 'NF{print $NF}')
if [ -z $container_name ];
then 
    echo ""
else
...
docker exec -it ${container_name} sh -c "brctl addif br0 eth1"
cat routeur_inside_script2.sh | docker exec -i ${container_name} sh
